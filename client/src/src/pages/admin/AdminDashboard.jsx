      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
                ...
                {(() => {
                  try {
                    ...
                    for (let i = 29; i >= 0; i--) {
                      ...
                      vendasPorDia[dataStr] = { data: dataStr, vendas: 0, pedidos: 0 };
                    }

                    pedidos.forEach(pedido => {
                      if (pedido && pedido.status === 'aprovado' && pedido.created_at) {
                        try {
                          const dataPedido = new Date(pedido.created_at).toISOString().split('T')[0];
                          if (vendasPorDia[dataPedido]) {
                            const totalPedido = (pedido.pedido_itens || []).reduce((sum, item) => {
                              if (!item) return sum;
                              return sum + (parseFloat(item.preco || 0) * (parseInt(item.quantidade) || 0));
                            }, 0);
                            vendasPorDia[dataPedido].vendas += totalPedido;
                            vendasPorDia[dataPedido].pedidos += 1;
                          }
                        } catch (e) {
                    ...
                    const diasOrdenados = Object.values(vendasPorDia).sort((a, b) => {
                      try {
                    ...
                    const maxVendas = Math.max(...diasOrdenados.map(d => d.vendas || 0), 1);
                      ...
                      <div className="space-y-2">
                        {diasOrdenados.map((dia, index) => {
                          try {
                            ...
                            const porcentagem = maxVendas > 0 ? ((dia.vendas || 0) / maxVendas) * 100 : 0;
                            
                            return (
                              <div key={index} className="flex items-center gap-4">
                                <div className="w-20 text-sm text-gray-600 font-medium">{dataFormatada}</div>
                                  ...
                                  >
                                    {dia.vendas > 0 && (
                                      <span className="text-white text-xs font-semibold">
                                        R$ {(dia.vendas || 0).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, '.')}
                                      </span>
                                    )}
                                  </div>
                                </div>
                                <div className="w-16 text-sm text-gray-600 text-right">
                                  {dia.pedidos || 0} {(dia.pedidos || 0) === 1 ? 'pedido' : 'pedidos'}
                                </div>
                              </div>
                            );
                          } catch (e) {
                            console.error('Erro ao renderizar dia:', e);
                            return null;
                          }
                        })}
                      </div>
                    ...
                    console.error('Erro ao processar vendas diárias:', error);
                    return <p className="text-red-500 text-center py-4">Erro ao carregar dados. Tente recarregar a página.</p>;
                  }
                })()}
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h2 className="text-xl font-bold mb-4">Funcionários Cadastrados por Empresa</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {empresas && empresas.length > 0 ? empresas.map(empresa => {
                  const funcionariosEmpresa = new Set();
                  if (pedidos && pedidos.length > 0) {
                    pedidos.forEach(pedido => {
                      const empresaId = pedido.funcionarios?.empresas?.id || pedido.funcionarios?.empresa_id;
                      if (empresaId === empresa.id && pedido.funcionarios?.id) {
                        funcionariosEmpresa.add(pedido.funcionarios.id);
                      }
                    });
                  }
                  ...
                  return (
                    <div key={empresa.id} className="border border-gray-200 rounded-lg p-4">
                      <h3 className="font-semibold text-gray-900 mb-2">{empresa.nome}</h3>
                      <p className="text-2xl font-bold text-primary-purple">{funcionariosEmpresa.size}</p>
                      <p className="text-xs text-gray-500 mt-1">funcionários com pedidos</p>
                    </div>
                  );
                }) : (
                  <p className="text-gray-500 text-center py-4 col-span-full">Nenhuma empresa cadastrada</p>
                )}
              </div>
            </div>
          </div>
